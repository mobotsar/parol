// ---------------------------------------------------------
// This file was generated by parol.
// It is not intended for manual editing and changes will be
// lost after next build.
// ---------------------------------------------------------

use id_tree::Tree;
use miette::Result;
use parol_runtime::lexer::{TokenStream, Tokenizer};
use parol_runtime::parser::{
    DFATransition, LLKParser, LookaheadDFA, ParseTreeType, ParseType, Production, UserActionsTrait,
};
use std::cell::RefCell;
use std::path::Path;

use parol_runtime::lexer::tokenizer::{
    ERROR_TOKEN, NEW_LINE_TOKEN, UNMATCHABLE_TOKEN, WHITESPACE_TOKEN,
};

pub const TERMINALS: &[&str; 32] = &[
    /*  0 */ UNMATCHABLE_TOKEN,
    /*  1 */ UNMATCHABLE_TOKEN,
    /*  2 */ UNMATCHABLE_TOKEN,
    /*  3 */ UNMATCHABLE_TOKEN,
    /*  4 */ UNMATCHABLE_TOKEN,
    /*  5 */ r###"%start"###,
    /*  6 */ r###"%title"###,
    /*  7 */ r###"%comment"###,
    /*  8 */ r###"%line_comment"###,
    /*  9 */ r###"%block_comment"###,
    /* 10 */ r###"%auto_newline_off"###,
    /* 11 */ r###"%auto_ws_off"###,
    /* 12 */ r###"%%"###,
    /* 13 */ r###":"###,
    /* 14 */ r###";"###,
    /* 15 */ r###"\|"###,
    /* 16 */ r###"<"###,
    /* 17 */ r###">"###,
    /* 18 */ r###"\("###,
    /* 19 */ r###"\)"###,
    /* 20 */ r###"\["###,
    /* 21 */ r###"\]"###,
    /* 22 */ r###"\{"###,
    /* 23 */ r###"\}"###,
    /* 24 */ r###"[a-zA-Z_][a-zA-Z0-9_]*"###,
    /* 25 */ r###"\u{0022}([^\\]|\\.)*?\u{0022}"###,
    /* 26 */ r###"%scanner"###,
    /* 27 */ r###","###,
    /* 28 */ r###"%sc"###,
    /* 29 */ r###"%push"###,
    /* 30 */ r###"%pop"###,
    /* 31 */ ERROR_TOKEN,
];

pub const TERMINAL_NAMES: &[&str; 32] = &[
    /*  0 */ "EndOfInput",
    /*  1 */ "Newline",
    /*  2 */ "Whitespace",
    /*  3 */ "LineComment",
    /*  4 */ "BlockComment",
    /*  5 */ "PercentStart",
    /*  6 */ "PercentTitle",
    /*  7 */ "PercentComment",
    /*  8 */ "PercentLineUnderscoreComment",
    /*  9 */ "PercentBlockUnderscoreComment",
    /* 10 */ "PercentAutoUnderscoreNewlineUnderscoreOff",
    /* 11 */ "PercentAutoUnderscoreWsUnderscoreOff",
    /* 12 */ "PercentPercent",
    /* 13 */ "Colon",
    /* 14 */ "Semicolon",
    /* 15 */ "Or",
    /* 16 */ "LT",
    /* 17 */ "GT",
    /* 18 */ "LParen",
    /* 19 */ "RParen",
    /* 20 */ "LBracket",
    /* 21 */ "RBracket",
    /* 22 */ "LBrace",
    /* 23 */ "RBrace",
    /* 24 */ "Identifier",
    /* 25 */ "String",
    /* 26 */ "PercentScanner",
    /* 27 */ "Comma",
    /* 28 */ "PercentSc",
    /* 29 */ "PercentPush",
    /* 30 */ "PercentPop",
    /* 31 */ "Error",
];

/* SCANNER_0: "INITIAL" */
const SCANNER_0: (&[&str; 5], &[usize; 26]) = (
    &[
        /*  0 */ UNMATCHABLE_TOKEN,
        /*  1 */ NEW_LINE_TOKEN,
        /*  2 */ WHITESPACE_TOKEN,
        /*  3 */ r###"(//.*(\r\n|\r|\n|$))"###,
        /*  4 */ r###"((?ms)/\*.*?\*/)"###,
    ],
    &[
        5,  /* PercentStart */
        6,  /* PercentTitle */
        7,  /* PercentComment */
        8,  /* PercentLineUnderscoreComment */
        9,  /* PercentBlockUnderscoreComment */
        10, /* PercentAutoUnderscoreNewlineUnderscoreOff */
        11, /* PercentAutoUnderscoreWsUnderscoreOff */
        12, /* PercentPercent */
        13, /* Colon */
        14, /* Semicolon */
        15, /* Or */
        16, /* LT */
        17, /* GT */
        18, /* LParen */
        19, /* RParen */
        20, /* LBracket */
        21, /* RBracket */
        22, /* LBrace */
        23, /* RBrace */
        24, /* Identifier */
        25, /* String */
        26, /* PercentScanner */
        27, /* Comma */
        28, /* PercentSc */
        29, /* PercentPush */
        30, /* PercentPop */
    ],
);

const MAX_K: usize = 1;

pub const NON_TERMINALS: &[&str; 30] = &[
    /*  0 */ "Alternation",
    /*  1 */ "AlternationList",
    /*  2 */ "Alternations",
    /*  3 */ "AlternationsList",
    /*  4 */ "Declaration",
    /*  5 */ "Factor",
    /*  6 */ "GrammarDefinition",
    /*  7 */ "GrammarDefinitionList",
    /*  8 */ "Group",
    /*  9 */ "Identifier",
    /* 10 */ "Optional",
    /* 11 */ "Parol",
    /* 12 */ "Production",
    /* 13 */ "Prolog",
    /* 14 */ "PrologList",
    /* 15 */ "PrologList1",
    /* 16 */ "Repeat",
    /* 17 */ "ScannerDirectives",
    /* 18 */ "ScannerState",
    /* 19 */ "ScannerStateList",
    /* 20 */ "ScannerSwitch",
    /* 21 */ "ScannerSwitchOpt",
    /* 22 */ "ScannerSwitchSuffix",
    /* 23 */ "SimpleToken",
    /* 24 */ "StartDeclaration",
    /* 25 */ "StateList",
    /* 26 */ "StateListList",
    /* 27 */ "String",
    /* 28 */ "Symbol",
    /* 29 */ "TokenWithStates",
];

pub const LOOKAHEAD_AUTOMATA: &[LookaheadDFA; 30] = &[
    /* 0 - "Alternation" */
    LookaheadDFA {
        states: &[Some(21)],
        transitions: &[],
        k: 0,
    },
    /* 1 - "AlternationList" */
    LookaheadDFA {
        states: &[None, Some(22), Some(23)],
        transitions: &[
            DFATransition(0, 14, 2),
            DFATransition(0, 15, 2),
            DFATransition(0, 16, 1),
            DFATransition(0, 18, 1),
            DFATransition(0, 19, 2),
            DFATransition(0, 20, 1),
            DFATransition(0, 21, 2),
            DFATransition(0, 22, 1),
            DFATransition(0, 23, 2),
            DFATransition(0, 24, 1),
            DFATransition(0, 25, 1),
            DFATransition(0, 28, 1),
            DFATransition(0, 29, 1),
            DFATransition(0, 30, 1),
        ],
        k: 1,
    },
    /* 2 - "Alternations" */
    LookaheadDFA {
        states: &[Some(18)],
        transitions: &[],
        k: 0,
    },
    /* 3 - "AlternationsList" */
    LookaheadDFA {
        states: &[None, Some(19), Some(20)],
        transitions: &[
            DFATransition(0, 14, 2),
            DFATransition(0, 15, 1),
            DFATransition(0, 19, 2),
            DFATransition(0, 21, 2),
            DFATransition(0, 23, 2),
        ],
        k: 1,
    },
    /* 4 - "Declaration" */
    LookaheadDFA {
        states: &[None, Some(7), Some(8), Some(9)],
        transitions: &[
            DFATransition(0, 6, 1),
            DFATransition(0, 7, 2),
            DFATransition(0, 8, 3),
            DFATransition(0, 9, 3),
            DFATransition(0, 10, 3),
            DFATransition(0, 11, 3),
        ],
        k: 1,
    },
    /* 5 - "Factor" */
    LookaheadDFA {
        states: &[None, Some(24), Some(25), Some(26), Some(27)],
        transitions: &[
            DFATransition(0, 16, 4),
            DFATransition(0, 18, 1),
            DFATransition(0, 20, 3),
            DFATransition(0, 22, 2),
            DFATransition(0, 24, 4),
            DFATransition(0, 25, 4),
            DFATransition(0, 28, 4),
            DFATransition(0, 29, 4),
            DFATransition(0, 30, 4),
        ],
        k: 1,
    },
    /* 6 - "GrammarDefinition" */
    LookaheadDFA {
        states: &[Some(14)],
        transitions: &[],
        k: 0,
    },
    /* 7 - "GrammarDefinitionList" */
    LookaheadDFA {
        states: &[None, Some(15), Some(16)],
        transitions: &[DFATransition(0, 0, 2), DFATransition(0, 24, 1)],
        k: 1,
    },
    /* 8 - "Group" */
    LookaheadDFA {
        states: &[Some(34)],
        transitions: &[],
        k: 0,
    },
    /* 9 - "Identifier" */
    LookaheadDFA {
        states: &[Some(37)],
        transitions: &[],
        k: 0,
    },
    /* 10 - "Optional" */
    LookaheadDFA {
        states: &[Some(35)],
        transitions: &[],
        k: 0,
    },
    /* 11 - "Parol" */
    LookaheadDFA {
        states: &[Some(0)],
        transitions: &[],
        k: 0,
    },
    /* 12 - "Production" */
    LookaheadDFA {
        states: &[Some(17)],
        transitions: &[],
        k: 0,
    },
    /* 13 - "Prolog" */
    LookaheadDFA {
        states: &[Some(1)],
        transitions: &[],
        k: 0,
    },
    /* 14 - "PrologList" */
    LookaheadDFA {
        states: &[None, Some(4), Some(5)],
        transitions: &[
            DFATransition(0, 6, 1),
            DFATransition(0, 7, 1),
            DFATransition(0, 8, 1),
            DFATransition(0, 9, 1),
            DFATransition(0, 10, 1),
            DFATransition(0, 11, 1),
            DFATransition(0, 12, 2),
            DFATransition(0, 26, 2),
        ],
        k: 1,
    },
    /* 15 - "PrologList1" */
    LookaheadDFA {
        states: &[None, Some(2), Some(3)],
        transitions: &[DFATransition(0, 12, 2), DFATransition(0, 26, 1)],
        k: 1,
    },
    /* 16 - "Repeat" */
    LookaheadDFA {
        states: &[Some(36)],
        transitions: &[],
        k: 0,
    },
    /* 17 - "ScannerDirectives" */
    LookaheadDFA {
        states: &[None, Some(10), Some(11), Some(12), Some(13)],
        transitions: &[
            DFATransition(0, 8, 1),
            DFATransition(0, 9, 2),
            DFATransition(0, 10, 3),
            DFATransition(0, 11, 4),
        ],
        k: 1,
    },
    /* 18 - "ScannerState" */
    LookaheadDFA {
        states: &[Some(39)],
        transitions: &[],
        k: 0,
    },
    /* 19 - "ScannerStateList" */
    LookaheadDFA {
        states: &[None, Some(40), Some(41)],
        transitions: &[
            DFATransition(0, 8, 1),
            DFATransition(0, 9, 1),
            DFATransition(0, 10, 1),
            DFATransition(0, 11, 1),
            DFATransition(0, 23, 2),
        ],
        k: 1,
    },
    /* 20 - "ScannerSwitch" */
    LookaheadDFA {
        states: &[None, Some(45), Some(48), Some(49)],
        transitions: &[
            DFATransition(0, 28, 1),
            DFATransition(0, 29, 2),
            DFATransition(0, 30, 3),
        ],
        k: 1,
    },
    /* 21 - "ScannerSwitchOpt" */
    LookaheadDFA {
        states: &[Some(50)],
        transitions: &[],
        k: 0,
    },
    /* 22 - "ScannerSwitchSuffix" */
    LookaheadDFA {
        states: &[None, Some(46), Some(47)],
        transitions: &[DFATransition(0, 19, 2), DFATransition(0, 24, 1)],
        k: 1,
    },
    /* 23 - "SimpleToken" */
    LookaheadDFA {
        states: &[Some(32)],
        transitions: &[],
        k: 0,
    },
    /* 24 - "StartDeclaration" */
    LookaheadDFA {
        states: &[Some(6)],
        transitions: &[],
        k: 0,
    },
    /* 25 - "StateList" */
    LookaheadDFA {
        states: &[Some(42)],
        transitions: &[],
        k: 0,
    },
    /* 26 - "StateListList" */
    LookaheadDFA {
        states: &[None, Some(43), Some(44)],
        transitions: &[DFATransition(0, 17, 2), DFATransition(0, 27, 1)],
        k: 1,
    },
    /* 27 - "String" */
    LookaheadDFA {
        states: &[Some(38)],
        transitions: &[],
        k: 0,
    },
    /* 28 - "Symbol" */
    LookaheadDFA {
        states: &[None, Some(28), Some(29), Some(30), Some(31)],
        transitions: &[
            DFATransition(0, 16, 3),
            DFATransition(0, 24, 1),
            DFATransition(0, 25, 2),
            DFATransition(0, 28, 4),
            DFATransition(0, 29, 4),
            DFATransition(0, 30, 4),
        ],
        k: 1,
    },
    /* 29 - "TokenWithStates" */
    LookaheadDFA {
        states: &[Some(33)],
        transitions: &[],
        k: 0,
    },
];

pub const PRODUCTIONS: &[Production; 51] = &[
    // 0 - Parol: Prolog GrammarDefinition;
    Production {
        lhs: 11,
        production: &[ParseType::N(6), ParseType::N(13)],
    },
    // 1 - Prolog: StartDeclaration PrologList PrologList1;
    Production {
        lhs: 13,
        production: &[ParseType::N(15), ParseType::N(14), ParseType::N(24)],
    },
    // 2 - PrologList1: ScannerState PrologList1;
    Production {
        lhs: 15,
        production: &[ParseType::N(15), ParseType::N(18)],
    },
    // 3 - PrologList1: Vec<PrologList1>::New;
    Production {
        lhs: 15,
        production: &[],
    },
    // 4 - PrologList: Declaration PrologList;
    Production {
        lhs: 14,
        production: &[ParseType::N(14), ParseType::N(4)],
    },
    // 5 - PrologList: Vec<PrologList>::New;
    Production {
        lhs: 14,
        production: &[],
    },
    // 6 - StartDeclaration: "%start" Identifier;
    Production {
        lhs: 24,
        production: &[ParseType::N(9), ParseType::T(5)],
    },
    // 7 - Declaration: "%title" String;
    Production {
        lhs: 4,
        production: &[ParseType::N(27), ParseType::T(6)],
    },
    // 8 - Declaration: "%comment" String;
    Production {
        lhs: 4,
        production: &[ParseType::N(27), ParseType::T(7)],
    },
    // 9 - Declaration: ScannerDirectives;
    Production {
        lhs: 4,
        production: &[ParseType::N(17)],
    },
    // 10 - ScannerDirectives: "%line_comment" String;
    Production {
        lhs: 17,
        production: &[ParseType::N(27), ParseType::T(8)],
    },
    // 11 - ScannerDirectives: "%block_comment" String String;
    Production {
        lhs: 17,
        production: &[ParseType::N(27), ParseType::N(27), ParseType::T(9)],
    },
    // 12 - ScannerDirectives: "%auto_newline_off";
    Production {
        lhs: 17,
        production: &[ParseType::T(10)],
    },
    // 13 - ScannerDirectives: "%auto_ws_off";
    Production {
        lhs: 17,
        production: &[ParseType::T(11)],
    },
    // 14 - GrammarDefinition: "%%" Production GrammarDefinitionList;
    Production {
        lhs: 6,
        production: &[ParseType::N(7), ParseType::N(12), ParseType::T(12)],
    },
    // 15 - GrammarDefinitionList: Production GrammarDefinitionList;
    Production {
        lhs: 7,
        production: &[ParseType::N(7), ParseType::N(12)],
    },
    // 16 - GrammarDefinitionList: Vec<GrammarDefinitionList>::New;
    Production {
        lhs: 7,
        production: &[],
    },
    // 17 - Production: Identifier ":" Alternations ";";
    Production {
        lhs: 12,
        production: &[
            ParseType::T(14),
            ParseType::N(2),
            ParseType::T(13),
            ParseType::N(9),
        ],
    },
    // 18 - Alternations: Alternation AlternationsList;
    Production {
        lhs: 2,
        production: &[ParseType::N(3), ParseType::N(0)],
    },
    // 19 - AlternationsList: "\|" Alternation AlternationsList;
    Production {
        lhs: 3,
        production: &[ParseType::N(3), ParseType::N(0), ParseType::T(15)],
    },
    // 20 - AlternationsList: Vec<AlternationsList>::New;
    Production {
        lhs: 3,
        production: &[],
    },
    // 21 - Alternation: AlternationList;
    Production {
        lhs: 0,
        production: &[ParseType::N(1)],
    },
    // 22 - AlternationList: Factor AlternationList;
    Production {
        lhs: 1,
        production: &[ParseType::N(1), ParseType::N(5)],
    },
    // 23 - AlternationList: Vec<AlternationList>::New;
    Production {
        lhs: 1,
        production: &[],
    },
    // 24 - Factor: Group;
    Production {
        lhs: 5,
        production: &[ParseType::N(8)],
    },
    // 25 - Factor: Repeat;
    Production {
        lhs: 5,
        production: &[ParseType::N(16)],
    },
    // 26 - Factor: Optional;
    Production {
        lhs: 5,
        production: &[ParseType::N(10)],
    },
    // 27 - Factor: Symbol;
    Production {
        lhs: 5,
        production: &[ParseType::N(28)],
    },
    // 28 - Symbol: Identifier;
    Production {
        lhs: 28,
        production: &[ParseType::N(9)],
    },
    // 29 - Symbol: SimpleToken;
    Production {
        lhs: 28,
        production: &[ParseType::N(23)],
    },
    // 30 - Symbol: TokenWithStates;
    Production {
        lhs: 28,
        production: &[ParseType::N(29)],
    },
    // 31 - Symbol: ScannerSwitch;
    Production {
        lhs: 28,
        production: &[ParseType::N(20)],
    },
    // 32 - SimpleToken: String;
    Production {
        lhs: 23,
        production: &[ParseType::N(27)],
    },
    // 33 - TokenWithStates: "<" StateList ">" String;
    Production {
        lhs: 29,
        production: &[
            ParseType::N(27),
            ParseType::T(17),
            ParseType::N(25),
            ParseType::T(16),
        ],
    },
    // 34 - Group: "\(" Alternations "\)";
    Production {
        lhs: 8,
        production: &[ParseType::T(19), ParseType::N(2), ParseType::T(18)],
    },
    // 35 - Optional: "\[" Alternations "\]";
    Production {
        lhs: 10,
        production: &[ParseType::T(21), ParseType::N(2), ParseType::T(20)],
    },
    // 36 - Repeat: "\{" Alternations "\}";
    Production {
        lhs: 16,
        production: &[ParseType::T(23), ParseType::N(2), ParseType::T(22)],
    },
    // 37 - Identifier: "[a-zA-Z_][a-zA-Z0-9_]*";
    Production {
        lhs: 9,
        production: &[ParseType::T(24)],
    },
    // 38 - String: "\u{0022}([^\\]|\\.)*?\u{0022}";
    Production {
        lhs: 27,
        production: &[ParseType::T(25)],
    },
    // 39 - ScannerState: "%scanner" Identifier "\{" ScannerStateList "\}";
    Production {
        lhs: 18,
        production: &[
            ParseType::T(23),
            ParseType::N(19),
            ParseType::T(22),
            ParseType::N(9),
            ParseType::T(26),
        ],
    },
    // 40 - ScannerStateList: ScannerDirectives ScannerStateList;
    Production {
        lhs: 19,
        production: &[ParseType::N(19), ParseType::N(17)],
    },
    // 41 - ScannerStateList: Vec<ScannerStateList>::New;
    Production {
        lhs: 19,
        production: &[],
    },
    // 42 - StateList: Identifier StateListList;
    Production {
        lhs: 25,
        production: &[ParseType::N(26), ParseType::N(9)],
    },
    // 43 - StateListList: "," Identifier StateListList;
    Production {
        lhs: 26,
        production: &[ParseType::N(26), ParseType::N(9), ParseType::T(27)],
    },
    // 44 - StateListList: Vec<StateListList>::New;
    Production {
        lhs: 26,
        production: &[],
    },
    // 45 - ScannerSwitch: "%sc" "\(" ScannerSwitchSuffix;
    Production {
        lhs: 20,
        production: &[ParseType::N(22), ParseType::T(18), ParseType::T(28)],
    },
    // 46 - ScannerSwitchSuffix: ScannerSwitchOpt "\)";
    Production {
        lhs: 22,
        production: &[ParseType::T(19), ParseType::N(21)],
    },
    // 47 - ScannerSwitchSuffix: Option<ScannerSwitchOpt>::None "\)";
    Production {
        lhs: 22,
        production: &[ParseType::T(19)],
    },
    // 48 - ScannerSwitch: "%push" "\(" Identifier "\)";
    Production {
        lhs: 20,
        production: &[
            ParseType::T(19),
            ParseType::N(9),
            ParseType::T(18),
            ParseType::T(29),
        ],
    },
    // 49 - ScannerSwitch: "%pop" "\(" "\)";
    Production {
        lhs: 20,
        production: &[ParseType::T(19), ParseType::T(18), ParseType::T(30)],
    },
    // 50 - ScannerSwitchOpt: Identifier;
    Production {
        lhs: 21,
        production: &[ParseType::N(9)],
    },
];

lazy_static! {
    static ref TOKENIZERS: Vec<(&'static str, Tokenizer)> = vec![(
        "INITIAL",
        Tokenizer::build(TERMINALS, SCANNER_0.0, SCANNER_0.1).unwrap()
    ),];
}

pub fn parse<'t, T>(
    input: &'t str,
    file_name: T,
    user_actions: &mut dyn UserActionsTrait,
) -> Result<Tree<ParseTreeType<'t>>>
where
    T: AsRef<Path>,
{
    let mut llk_parser = LLKParser::new(
        11,
        LOOKAHEAD_AUTOMATA,
        PRODUCTIONS,
        TERMINAL_NAMES,
        NON_TERMINALS,
    );
    let token_stream =
        RefCell::new(TokenStream::new(input, file_name, &TOKENIZERS, MAX_K).unwrap());
    let result = llk_parser.parse(token_stream, user_actions);
    match result {
        Ok(()) => Ok(llk_parser.parse_tree),
        Err(e) => Err(e),
    }
}
